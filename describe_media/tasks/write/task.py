...
class WriteTask(Task[Tuple[str, Union[str, Exception], Optional[Dict[str, Any]]], str]):
    ...
    def execute(self, item: Tuple[str, Union[str, Exception], Optional[Dict[str, Any]]]) -> str:
        ...
        if self.input_dir and self.output_dir:
            relative = output_relative_path(input_path, self.input_dir, metadata or {})
        ...
        if isinstance(content_or_error, Exception):
            ...
            try:
                ...
                if isinstance(metadata, dict) and metadata.get('datetime'):
                ...
                if isinstance(metadata, dict) and metadata.get('location_str'):
                ...
                if isinstance(metadata, dict) and metadata.get('camera'):
