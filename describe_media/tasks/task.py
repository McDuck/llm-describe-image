...
class Task(Generic[InputType, OutputType]):
    ...
    def get_preferred_image_path(input_path: str) -> str:
        """
        Check if a .resized-*p.jpg, .fixed, or original version of the image exists and return the best one.
        Priority: resized > fixed > original
        
        Used to prefer processed images over corrupted originals.
        
        Args:
            input_path: Original image path
        
        Returns:
            Path to preferred image
        """
        base, ext = os.path.splitext(input_path)
        
        # Check for resized version first
        resized_path = base + ".resized.jpg"
        if os.path.exists(resized_path):
            return resized_path
        
        # Then check for fixed version
