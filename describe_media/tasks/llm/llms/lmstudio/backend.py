...
class LMStudioBackend(LLMBackend):
    ...
    def _configure_client_host(self) -> None:
        """Allow Docker/local callers to override the LM Studio API host."""
        api_host = os.getenv("LMSTUDIO_API_HOST", "").strip()
        if api_host:
    ...
    def bootstrap_server(self, auto_start: bool, sync_api_timeout_s: int = 600) -> bool:
        self._configure_client_host()
        # Try SDK bootstrap - if it works, server is already running
        try:
            lms.bootstrap()
            # Set configurable timeout to prevent premature channel closures
            lms.set_sync_api_timeout(sync_api_timeout_s)
